...
if __name__ == '__main__':
    ...
    parser.add_argument('--data_path', default='/path/to/imagenet', type=str, help='Path to ImageNet dataset.')
    ...
    train_dataset = ImageNet(root=args.data_path, split='train', download=False, transform=transform)
    val_dataset = ImageNet(root=args.data_path, split='val', download=False, transform=transform)
    ...
    val_loader = DataLoader(val_dataset, batch_size=64, shuffle=False, num_workers=2, worker_init_fn=lambda _: np.random.seed(seed))
    ...
    # Build the model
    model = vit_register_dynamic_viz(img_size=224,  patch_size=16, in_chans=3, num_classes=1000, embed_dim=384, depth=12,
    ...
    with PdfPages(os.path.join(args.output_dir, "ImageNet_attention_maps.pdf")) as pdf:
