...
# Get 10 batches of training data for overfitting test
train_iterator = iter(train_loader)
batches = [next(train_iterator) for _ in range(2)]
inputs = torch.cat([batch[0] for batch in batches], dim=0).to(device)
targets = torch.cat([batch[1] for batch in batches], dim=0).to(device)
